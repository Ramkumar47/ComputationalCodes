...
def generate_dataset(N_bc,N_int):
    ...
    #  x_AB     = np.random.rand(N_bc)
    x_AB     = np.linspace(0,1,N_bc)
    y_AB     = x_AB*0.0
    theta_AB = x_AB*0.0

    #  y_BC     = np.random.rand(N_bc)
    y_BC     = np.linspace(0,1,N_bc)
    x_BC     = y_BC*0.0 + 1.0
    theta_BC = x_BC*0.0

    #  x_CD     = np.random.rand(N_bc)
    x_CD     = np.linspace(0,1,N_bc)
    y_CD     = x_CD*0.0 + 1.0
    theta_CD = x_CD*0.0

    #  y_DA     = np.random.rand(N_bc)
    ...
    #  x_bc     = np.r_[x_bc,np.array([0,1,1,0])]
    #  y_bc     = np.r_[y_bc,np.array([0,0,1,1])]
    #  theta_bc = np.r_[theta_bc,np.array([0.5,0,0,0.5])]
    ...
    #  x_int,y_int = np.random.rand(2,N_int)
    x_int,y_int = np.meshgrid(np.linspace(0.1,0.9,N_int),np.linspace(0.1,0.9,N_int))
    x_int = x_int.flatten()
    y_int = y_int.flatten()
    #  x_int = np.r_[x_int,x_bc]
    #  y_int = np.r_[y_int,y_bc]
...
# error computation
#  theta_error = np.abs(theta_act-theta_pred)/theta_act*100
numerator = np.abs(theta_act-theta_pred)
theta_error = np.divide(numerator,theta_act, out = np.zeros_like(numerator), where=theta_act!=0)
