...
N     = 101           # number of input function variations

# GENERATING DATA

# generating random omega and theta values
omega_array = np.random.rand(N)*omega[1]+omega[0]
theta_array = np.random.rand(P)*theta[1]+theta[0]
...
for i in range(N): # looping through number of input function variations
    for j in range(P): # looping through no. of output function locations
        ...
        output_func = 1/omega_array[i]*np.sin(omega_array[i]*theta_array[j])
        ...
        omega_list.append(omega_array[i])
...
fid.to_csv("computed_data.csv", index = None)
