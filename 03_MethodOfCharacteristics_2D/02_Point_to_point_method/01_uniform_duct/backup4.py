...
for i in range(N,N_total):
    ...
    if N_curr in pnts_bottom:
        ...
        S1 = np.tan(fid["theta"].iloc[n1]+fid["nu"].iloc[n1])

        # computing the x position of current char pnt
        #  x = (fid["X"].iloc[n1] - fid["Y"].iloc[n1])/(S1-np.sin(theta_lower))
        x = (fid["Y"].iloc[n1] - fid["X"].iloc[n1]*S1)/(np.sin(theta_lower)-S1)
    ...
    # checking if it is on top wall
        ...
        S2 = np.tan(fid["theta"].iloc[n2]-fid["nu"].iloc[n2])

        # computing the x position of current char pnt
        #  x = (height + fid["X"].iloc[n1] - fid["Y"].iloc[n1])/(S2-np.sin(theta_upper))
        x = (fid["Y"].iloc[n2] - fid["X"].iloc[n2] - height)/(np.sin(theta_upper)-S2)
