...
for i in range(N,N_total):
    ...
    if N_curr in pnts_bottom:
        # it has only K2 characteristics and theta as theta_lower
        theta = theta_lower
        ...
        x = (fid["X"].iloc[n2] - fid["Y"].iloc[n2])/(S2-np.sin(theta_lower))
    ...
    # checking if it is on top wall
        ...
        # it has only K1 characteristics and theta as theta_upper
        theta = theta_upper
        ...
        # computing the x position of current char pnt
        x = (height + fid["X"].iloc[n1] - fid["Y"].iloc[n1])/(S1-np.sin(theta_upper))
