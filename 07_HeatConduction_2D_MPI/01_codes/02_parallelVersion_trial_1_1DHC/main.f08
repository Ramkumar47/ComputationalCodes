...
program main
    ...
    mainloop: do iter = 1, maxIterCount
        ...
        if (proc_id == 0) then
            ...
            call MPI_send(T_chunk(N_chunk), 1, MPI_DOUBLE_PRECISION, 1, N_chunk, MPI_COMM_WORLD, ierror)

            ! receiving the boundary value from the other process
            call MPI_recv(T_bc, 1, MPI_DOUBLE_PRECISION, 1, N_chunk+1, MPI_COMM_WORLD, MPI_STATUS_IGNORE, ierror)
        ...
        if (proc_id == 1) then
            ...
            ! sending the edge value to the other process
            call MPI_send(T_chunk(1), 1, MPI_DOUBLE_PRECISION, 0, N_chunk+1, MPI_COMM_WORLD, ierror)

            ! receiving the boundary value from the other process
            call MPI_recv(T_bc, 1, MPI_DOUBLE_PRECISION, 0, N_chunk, MPI_COMM_WORLD, MPI_STATUS_IGNORE, ierror)
