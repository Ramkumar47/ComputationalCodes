...
let xRange = [-50,50]; // range of coordinates
let yRange = [-50,50];
let zRange = [-50,50];
...
let hueCoeff = 1.38; // hue slope coeff. for lyapunov coloring
let hueOff = -1.38; // hue slope coeff. for lyapunov coloring

// equation function definition
function f(xVal,yVal,zVal){
    xDot = xVal+xVal*yVal;
    yDot = -zVal;
    zDot = yVal-0.2*zVal+xVal**2;
...
function setup() {
    ...
    describe('Reflection Symmetric System',LABEL);
    frDiv = createDiv('');
