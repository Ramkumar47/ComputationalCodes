...
let xRange = [-10,10]; // range of coordinates
let yRange = [-10,10];
let zRange = [-10,10];
...
let hueCoeff = 6.57; // hue slope coeff. for lyapunov coloring
let hueOff = -1.64; // hue slope coeff. for lyapunov coloring

// equation function definition
function f(xVal,yVal,zVal){
    xDot = yVal;
    yDot = -xVal-zVal*yVal;
    zDot = yVal**2-4+0.1*zVal;
...
function setup() {
    ...
    describe('Attracting Torus System',LABEL);
    frDiv = createDiv('');
