// Lorenz-84 System
let dt = 2e-2; // simulation timestep
let Tstop = 50.0; // simulation end time

let x = [-5]; // initial conditions
let y = [-1];
let z = [-2];
let LL = [0]; // local lyapunov
let r0 = 1e-8; // perturbation value

let xRange = [-6,6]; // range of coordinates
let yRange = [-6,6];
let zRange = [-6,6];
...
let hueCoeff = 10.12; // hue slope coeff. for lyapunov coloring
let hueOff = 0.5; // hue slope coeff. for lyapunov coloring

// equation function definition
function f(xVal,yVal,zVal){
    xDot = -1*yVal**2-zVal**2-xVal+1;
    yDot = yVal-xVal*zVal-0.1*yVal+1;
    zDot = xVal*yVal+xVal*zVal-zVal;
...
function setup() {
    ...
    describe('Lorenz-84 System',LABEL);
    frDiv = createDiv('');
...
// absolute minimum function of an array
function absMinLL(){
    minVal = 1e5;
