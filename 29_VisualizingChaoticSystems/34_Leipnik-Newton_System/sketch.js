...
let dt = 1e-2; // simulation timestep
let Tstop = 20.0; // simulation end time
...
let xRange = [-16,16]; // range of coordinates
let yRange = [-16,16];
let zRange = [-16,16];
...
let hueCoeff = 6.37; // hue slope coeff. for lyapunov coloring
let hueOff = 19.10; // hue slope coeff. for lyapunov coloring

// equation function definition
function f(xVal,yVal,zVal){
    xDot = -xVal+yVal+yVal*zVal;
    yDot = -32*xVal-yVal+xVal*yVal;
    zDot = zVal-xVal*yVal;
...
function setup() {
    ...
    describe('Leipnik-Newton System',LABEL);
    frDiv = createDiv('');
