...
let Tstop = 200.0; // simulation end time

let x = [2]; // initial conditions
let y = [-1];
let z = [1];
let LL = [0]; // local lyapunov
let r0 = 1e-8; // perturbation value

let xRange = [-5,5]; // range of coordinates
let yRange = [-5,5];
let zRange = [-5,5];
...
let hueCoeff = 9.75; // hue slope coeff. for lyapunov coloring
let hueOff = 0; // hue slope coeff. for lyapunov coloring

// equation function definition
function f(xVal,yVal,zVal){
    xDot = yVal*zVal-yVal+0.6*xVal**2*yVal+xVal;
    yDot = xVal*zVal+xVal-xVal**3+0.07*yVal;
    zDot = -zVal-xVal*yVal*zVal;
...
function setup() {
    ...
    describe('Rabinovich-Fabrikant System',LABEL);
    frDiv = createDiv('');
