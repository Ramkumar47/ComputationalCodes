...
let xRange = [-7,7]; // range of coordinates
let yRange = [-7,7];
let zRange = [-7,7];
...
let hueCoeff = 17269.58; // hue slope coeff. for lyapunov coloring
let hueOff = -1692.42; // hue slope coeff. for lyapunov coloring

// equation function definition
function f(xVal,yVal,zVal){
    xDot = -0.1*xVal+sin(yVal);
    yDot = -0.1*yVal+sin(zVal);
    zDot = -0.1*zVal+sin(xVal);
