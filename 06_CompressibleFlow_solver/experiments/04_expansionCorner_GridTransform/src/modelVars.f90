...
module modelVars
    ...
    real(kind=rkd),dimension(Ny,Nx) :: Mach, S1,S2,S3,S5
    real(kind=rkd),dimension(Ny,Nx) :: Eps,Eta, Jac, dXdEps, dXdEta
    real(kind=rkd),dimension(Ny,Nx) :: dYdEps, dYdEta

    ! defining scalar variables
    real(kind=rkd) :: deltaT, time = 0, convergenceResidual = 1.0
    real(kind=rkd) :: dEta = 1.0, dEps = 1.0
