...
module parameters
    ...
    real(kind=rkd), parameter :: M_inf = 0.1
    ...
    integer(kind=ikd), parameter :: Nx = 201, Ny = 21

    ! domain length and height
    real(kind=rkd), parameter :: L_x = 4.3e-4, L_y = 4.3e-5

    ! max simulation time and timestep count
    real(kind=rkd), parameter :: maxSimTime = 1.0
    integer(kind=ikd), parameter :: maxTSCount = 100000
